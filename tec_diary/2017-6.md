...
这时候，子组件触发了一个事件 sendData, 并且通过事件传递数据 data 过去，父组件需要做的，就是使用 `v-on` 来侦听来自子组件的事件，并且通过自身事件来处理数据：
...
#### slot 内容分发
在父组件中调用子组件时，常常有来自父组件的内容需要插到子组件内部，这个时候就可以通过在子组件中定义 slot 插槽来决定父组件内容的混入位置，slot 可以提供一个 name 属性，以便于父组件不同内容与插槽位置的匹配（父组件需要分发的内容提供 slot 属性，与相应 name 的 slot 对应渲染）
